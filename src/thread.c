...
SIZEL           sizelRB={RB_BMPSIZE,RB_BMPSIZE};
...
for(i=1;i<RB_BORDER;i++)
   {
   if(!(hdcRB[i]=DevOpenDC(habDT,        /* Open device context — use drawing thread's HAB */
   ...
   if(!(hpsRB[i]=GpiCreatePS(habDT,     /* Create PS on drawing thread's HAB so GpiBitBlt
                                           can use it as source on this thread */
...
if(!(hpsDT=WinGetPS(hwndClient)))       /* Get a presentation space for client area */
    GEN_ERR(habDT,hwndFrame,hwndClient);
                                        /* Load bitmap resources on this thread so all
                                           PS/DC handles share the same HAB as hpsDT */
Draw_Initialize();
...
while(qmsqDT.msg!=DT_EXIT)
    ...
    switch(qmsqDT.msg)
    {
    case DT_INITIALIZE:                 /* Start new game from drawing thread */
     ...
     case DT_IDLE:
        ...
        if(runRB==TRUE)
            ...
            case RB_HOLE:               /* We got a hole, sorry but RollBall will be killed.
                ...
                WinPostMsg(hwndClient,WM_COMMAND,(MPARAM)IDM_QUIT,(MPARAM)0);
                break;
                }
            }
...
DosExit(EXIT_THREAD,0UL);
                                        ...
                                        /* Draw a bitmap on client window, scaled to RB_SIZE×RB_SIZE */
void Draw_Bitmap(ULONG Bitmap,ULONG mode,ULONG x,ULONG y)
{
RECTL rcl;
rcl.xLeft   = (LONG)x;
rcl.yBottom = (LONG)y;
rcl.xRight  = (LONG)(x + RB_SIZE);
rcl.yTop    = (LONG)(y + RB_SIZE);
(void)mode;
WinDrawBitmap(hpsDT,hbmRB[Bitmap],NULL,(PPOINTL)(void *)&rcl,
              CLR_NEUTRAL,CLR_BACKGROUND,DBM_NORMAL|DBM_STRETCH);
}
